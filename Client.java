...
public class Client {
    ...
    // NOTE: ServerHost must be updated to the actual Server IP address (if running
    // on diff LAN).
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            startClientSession(scanner, reader, writer, socket);

            // The Socket will automatically close when the main try block exits or the
    ...
    private static void startClientSession(Scanner scanner, BufferedReader reader, PrintWriter writer, Socket socket)
            throws IOException {
        while (true) { // Loop for multiple requests over the same connection.
            ...
            // Calculate RTT: (endTime - startTime) in nanoseconds, converted to
            // milliseconds (ms).
        ...
        // The socket streams will be implicitly closed upon exiting the try block or
        // main method.
    }
